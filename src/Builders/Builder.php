...
class Builder
{
    ...
    public function getByPage( $page = 0, $pageSize = 500, $filters = [] )
    {
        ...
        return $this->request->handleWithExceptions( function () use ( $pageSize, &$page, &$items, $urlFilters ) {
            $response =
                $this->request->curl->get( "/{$this->entity}?skippages={$page}&pagesize={$pageSize}{$urlFilters}" );
    ...
    public function all( $filters = [], $pageSize = 500 )
    {
        ...
        return $this->request->handleWithExceptions( function () use (
            &$hasMore, $pagesize, &$page, &$items, $urlFilters
        ) {
            while ( $hasMore ) {
                $response =
                    $this->request->curl->get( "/{$this->entity}?skippages={$page}&pagesize={$pagesize}{$urlFilters}" );
